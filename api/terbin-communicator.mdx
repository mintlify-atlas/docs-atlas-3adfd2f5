...
Handles payloads that exceed `TerbinProtocol.MAX_PLD` by splitting them into `FRAGMENT_IN`-byte (0xFFAA) chunks, requesting a remote memory slot via `SoliciteRequestMemory()`, uploading each fragment via `Load()`, and then sending the final packet.
...
```csharp
await using var communicator = new TerbinCommunicator(false, cts.Token);
