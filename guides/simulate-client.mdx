...
<Tip>
  The entire interactive block in `SimulateClient/Program.cs` is wrapped in a `#if true … #else … #endif` gate. If you change the `#if true` to `#if false`, the project will compile and run but will immediately print:
