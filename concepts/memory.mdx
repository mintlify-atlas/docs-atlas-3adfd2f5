...
sidebarTitle: "Memory"
description: "How TerbinMemory stores, orders, and reassembles byte fragments for large payloads that exceed the single-packet size limit."
---
...
Stores a fragment under its `OrderRequest` key. Duplicate order values are silently ignored (the first arrival wins). Increments `_totalSize` by the fragment length. Fires `OnAdd` after insertion.
...
| Event | When it fires |
|---|---|
| `OnAdd` | After each successful `AddFragment` call |
...
This is called by `TerbinExecutor.Solicit` when the remote side sends `CodeStatus.Execute` with `IdMemory = CodeTerbinMemory.New`.
