---
title: "BufferReader and BufferWriter: Low-Level Span Buffer API"
sidebarTitle: "Buffer Reader/Writer"
description: "API reference for BufferReader and BufferWriter — low-level Span APIs for reading and writing unmanaged types, arrays, and IStructSerializable structs."
---
...
<ParamField path="Add<T>(Span<byte> pBuffer, ref int pOffset, T pValue) where T : unmanaged" type="void">
  Writes a single unmanaged value at `pOffset` and advances `pOffset` by `sizeof(T)`.
</ParamField>

<ParamField path="AddArray<T>(Span<byte> pBuffer, ref int pOffset, T[] pArray) where T : unmanaged" type="void">
...
<ParamField path="AddStruct<T>(Span<byte> pBuffer, ref int pOffset, T pStruct) where T : struct, IStructSerializable" type="void">
  Writes an `IStructSerializable` struct by calling `pStruct.WriteTo()`.
</ParamField>

<ParamField path="EnsureAdd<T>(ref byte[] buffer, int offset, T value) where T : unmanaged" type="void">
...
<ParamField path="Get<T>(ReadOnlySpan<byte> pBuffer, ref int pOffset) where T : unmanaged" type="T">
  Reads a single unmanaged value at `pOffset` and advances `pOffset` by `sizeof(T)`.
</ParamField>

<ParamField path="GetArray<T>(ReadOnlySpan<byte> pBuffer, ref int pOffset) where T : unmanaged" type="T[]">
...
<ParamField path="GetStruct<T>(ReadOnlySpan<byte> pBuffer, ref int pOffset, T pStruct) where T : struct, IStructSerializable" type="T">
