---
title: "Named-Pipe Communication with TerbinCommunicator"
...
Enqueues the packet and returns immediately. If the payload is small enough (≤ `MAX_PLD`) this returns `null`. If fragmentation is required and the handshake or memory allocation fails, the returned `PacketRequest` contains the error status.
