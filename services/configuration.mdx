...
<Info>
  On first startup, `Manager.Configuration` calls `setPredeterminatedConfig()` which auto-detects and persists `rute_farlands`, `rute_instances`, and `rute_plugins`. The Steam and Proton keys have no automatic defaults and will return `AccesNullOrNotExist` until configured. The source also notes a TODO to manage Steam/Proton installation state and to move instances when their path changes.
</Info>
...
| `AccesNullOrNotExist` | The key is not configured and has no default. Payload is empty. |
