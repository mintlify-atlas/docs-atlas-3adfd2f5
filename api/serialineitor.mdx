...
description: "API reference for Serialineitor — the fluent builder for constructing binary payloads, with static helpers for serializing unmanaged types and arrays."
---
...
<ParamField path="Add<T>(T pValue) where T : unmanaged" type="Serialineitor">
  Appends an unmanaged value to the buffer. The buffer auto-grows if needed. Returns `this` for chaining.
</ParamField>

<ParamField path="AddArray<T>(T[] pArray) where T : unmanaged" type="Serialineitor">
  Appends a `ThreeQuartersInt` (3-byte) length prefix followed by the raw element bytes. Returns `this` for chaining.
</ParamField>

<ParamField path="AddStruct<T>(T pStruct) where T : struct, IStructSerializable" type="Serialineitor">
